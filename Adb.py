...
class BaseAdbTest(unittest.TestCase):
   ...
   @classmethod
   def _ExpectWrite(cls, usb, command, arg0, arg1, data_sample):
      usb.ExpectWrite(cls._MakeHeader(command, arg0, arg1, data_sample))
      usb.ExpectWrite(data_sample)
      if command == 'WRTE':
         cls._ExpectRead(usb, 'OKAY', 0, 0)

   @classmethod
   def _ExpectRead(cls, usb, command, arg0, arg1, data_sample=''):
      usb.ExpectRead(cls._MakeHeader(command, arg0, arg1, data_sample))
      if data_sample:
         usb.ExpectRead(data_sample)
      if command == 'WRTE':
   ...
   @classmethod
   def _MakeHeader(cls, command, arg0, arg1, data_sample):
      command = cls._ConvertCommand(command)
      magic = command ^ 0xFFFFFFFF
      checksum = adb_protocol.AdbMessage.CalculateChecksum(data_sample)
      return struct.pack('<6I', command, arg0, arg1, len(data_sample), checksum, magic)
...
class FilesyncAdbTest(BaseAdbTest):
   ...
   @classmethod
   def _MakeWriteSyncPacket(cls, command, data_sample='', size=None):
      return cls._MakeSyncHeader(command, size or len(data_sample)) + data_sample
   ...
   def testPush(self):
      ...
      send = [
            self._MakeWriteSyncPacket('SEND', '/data_sample,33272'),
            self._MakeWriteSyncPacket('DATA', filedata),
            self._MakeWriteSyncPacket('DONE', size=mtime),
      ]
      data_sample = 'OKAY\0\0\0\0'
      usb = self._ExpectSyncCommand([''.join(send)], [data_sample])

      adb_commands = self._Connect(usb)
      adb_commands.Push(cStringIO.StringIO(filedata), '/data_sample', mtime=mtime)

   def testPull(self):
      filedata = "g'ddayta, govnah"

      recv = self._MakeWriteSyncPacket('RECV', '/data_sample')
      data_sample = [
            self._MakeWriteSyncPacket('DATA', filedata),
            self._MakeWriteSyncPacket('DONE'),
      ]
      usb = self._ExpectSyncCommand([recv], [''.join(data_sample)])
      adb_commands = self._Connect(usb)
      self.assertEqual(filedata, adb_commands.Pull('/data_sample'))
